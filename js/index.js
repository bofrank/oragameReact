    ...
    if(edamamJSON.hits.length > 0) {
      for(k = 0; k < edamamJSON.hits.length; k++){
        ...
        for(i = 0; i < edamamJSON.hits[k].recipe.ingredients.length; i++){
          ingredientsHTML += '<div itemprop="recipeIngredient">'+edamamJSON.hits[k].recipe.ingredients[i].text+'</div>';
        }

        recipeHTML = '<a itemscope itemtype="http://schema.org/Recipe" id="recipe"'+k+' class="item photo" href="'+edamamJSON.hits[k].recipe.url+'" target="_blank"><div class="content"><img itemprop="image" id="image0"'+k+' class="grid-item-image" src="'+edamamJSON.hits[k].recipe.image+'" onload="resizeAllGridItems()" /><div itemprop="name" id="title0"'+k+' class="grid-item-title">'+edamamJSON.hits[k].recipe.label+'</div><div class="grid-text">'+ingredientsHTML+'</div></div></a>';
      ...
      document.getElementById("grid").innerHTML +=  " "+ingredient+" is not found in our recipe library :(";
    }
